...
For example, if `sample=100` vectors are selected to form a subset, and `limit=10` similar vectors are searched for each, the returned distance matrix will be a {{< katex >}}\\(100 \\times 10\\) matrix, where each row represents the top 10 similar vectors for one point.  
