...
## 前言
基于向量数据库的 RAG 是最基础的 RAG 形式，在实际生产中被广泛应用。正如其名，向量数据库主要存储的元素是一个个的向量，通过向量运算，我们能够实现更加精准地**相关性查询**，这个功能是一些应用场景地**基石**。
...
5. **批量搜索**：一次性输入一组向量进行查询
6. **搜索分组**：能够对搜索结果按照某些标签进行分组，参数 `group_size` 可以设置每组的结果个数
7. **查询规划**：依据可选索引、过滤条件的复杂性和总共的数据点的数量，以启发式的方法选择一个合适的查询偏好(提高性能🤔)
